...
class WfcImpl {
    ...
    onGroupMemberUpdateListener(groupId) {
        ...
        self.eventEmitter.emit(EventType.GroupMembersUpdate, groupId);
    }
    ...
    onPinUnpinMessage(message){
        ...
        setTimeout(() => {
            if(isPinned){
                console.log("GOTO***PIN")
                self.eventEmitter.emit(EventType.PinMessage, stringValue(message.messageUid));
                // self.$eventBus.$emit('pin-unpin-change', true);
                return;
            }else{
                console.log("GOTO-UNPIN")
    ...
    getUploadMediaUrl(fileName, mediaType, successCB, failCB) {
        proto.getUploadMediaUrl(fileName, mediaType, successCB, failCB);
    }
    ...
    searchFiles(keyword, conversation, fromUser, beforeMessageId, count, successCB, failCB) {
        proto.searchFiles(keyword, conversation ? JSON.stringify(conversation) : '', fromUser, Long.fromValue(beforeMessageId).toString(), count,
            (files) => {
    ...
    searchMyFiles(keyword, beforeMessageUid, count, successCB, failCB) {
        proto.searchMyFiles(keyword, Long.fromValue(beforeMessageUid).toString(), count,
            (files) => {
                let frs = JSON.parse(files);
