...
class WfcImpl {
    ...
    onPinUnpinMessage(messageId, pinStatus){
        if (!self.isLogined) {
            return;
        }
        if(pinStatus){
            self.eventEmitter.emit(EventType.PinMessage, messageId);
        }else{
            self.eventEmitter.emit(EventType.unpinMessage, messageId);
        }
    }
    ...
    async pinUnpinMessage(messageUid, pinStatus, messageContent, successCB, failCB) {
        console.log("MMM", messageContent)
        // let protoMessageContent = messageContent.encode();
        // console.log("GBV", this.updateMessageContent(messageUid, messageContent))
        // this.onPinUnpinMessage(messageUid, pinStatus)

        proto.updateRemoteMessageContent(messageUid.toString(), JSON.stringify(messageContent), true, true,
        () => {
            if (successCB) {
                successCB();
                this.onPinUnpinMessage(messageUid, pinStatus)
