...
class WfcImpl {
    ...
    onReceiveMessage(messages, hasMore) {
        ...
        msgs.forEach(m => {
            ...
            if (msg.messageContent instanceof MarkUnreadMessageContent) {
                let markMsg = msg.messageContent;
                let conversation = msg.conversation;
                proto.setLastReceivedMessageUnRead(JSON.stringify(conversation), stringValue(markMsg.messageUid), stringValue(markMsg.timestamp));
            }
            if (msg) {
    ...
    onGroupMemberUpdateListener(groupId, groupMembersStr) {
        ...
        let arr;
        try {
            arr = JSON.parse(groupMembersStr);
        } catch (error) {
        }
        if(arr){
            arr.forEach(e => {
    ...
    onPinUnpinMessage(message){
        if (!self.isLogined) {
            return;
        }
        const isPinned = JSON.parse(message.messageContent.extra).isPinned ? true : false;
        setTimeout(() => {
            if(isPinned){
                self.eventEmitter.emit(EventType.PinMessage, stringValue(message.messageUid));
                return;
            }else{
                self.eventEmitter.emit(EventType.UnpinMessage, stringValue(message.messageUid));
                return;
            }
        }, 10);
    ...
    init(args = []) {
        ...
        // }
        // proto.setAppName('mychat');
        proto.setConnectionStatusListener(self.onConnectionChanged);
        // proto.setConnectToServerListener(self.onConnectToServer);
    ...
    registerMessageContent(name, flag, type, clazz) {
        ...
        proto.registerMessageFlag(type, flag);
    }
    ...
    setProxyInfo(host, ip, port, username, password) {
        proto.setProxyInfo(host, ip, port, username, password);
    }
    ...
    addHttpHeader(header, value) {
        ...
        proto.addHttpHeader(header, value);
    }
    ...
    getFriendList(fresh = false) {
        let result = proto.getFriendList(fresh);
        return JSON.parse(result);
    }
    ...
    async createGroup(groupId, groupType, name, portrait, groupExtra = '', memberIds = [], memberExtra = '', lines = [0], notifyContent, successCB, failCB) {
        ...
        proto.createGroup(groupId, groupType, name, portrait, groupExtra, memberIds, memberExtra, lines, notifyContentStr,
            (groupId) => {
                if (successCB) {
                    successCB(groupId);
                }
    ...
    addGroupMembers(groupId, memberIds, extra, notifyLines, notifyMessageContent, successCB, failCB) {
        ...
        let notifyContentStr = JSON.stringify(payload);
        extra = !extra ? '' : extra;
        proto.addMembers(memberIds, groupId, extra, notifyLines, notifyContentStr,
            () => {
                if (successCB) {
                    successCB();
                }
    ...
    async modifyGroupMemberAlias(groupId, memberId, alias, lines = [0], notifyMessageContent, successCB, failCB) {
        let payload = notifyMessageContent ? notifyMessageContent.encode() : '';
        proto.modifyGroupMemberAlias(groupId, memberId, alias, lines, JSON.stringify(payload), () => {
            successCB();
        }, (errorCode) => {
            failCB(errorCode);
        });
    ...
    async modifyGroupMemberExtra(groupId, memberId, extra, lines = [0], notifyMessageContent, successCB, failCB) {
        let payload = notifyMessageContent ? notifyMessageContent.encode() : '';
        proto.modifyGroupMemberExtra(groupId, memberId, extra, lines, JSON.stringify(payload), () => {
            successCB();
        }, (errorCode) => {
            failCB(errorCode);
        });
    ...
    createChannel(name, portrait, desc, extra, successCB, failCB) {
        proto.createChannel(name, portrait, 0, desc, extra, (info) => {
            if (successCB) {
    ...
    markConversationAsUnread(conversation, syncToOtherClient) {
        let msgUid = proto.setLastReceivedMessageUnRead(JSON.stringify(conversation), '0', '0');
        if (msgUid && gt(msgUid, 0) && syncToOtherClient) {
    ...
    setMessageLocalExtra(messageId, extra) {
        proto.setMessageLocalExtra(messageId, extra);
    }
    ...
    async sendFriendRequest(userId, reason, extra, successCB, failCB) {
        extra = !extra ? '' : extra;
        proto.sendFriendRequest(userId, reason, extra, () => {
            if (successCB) {
                successCB();
            }
    ...
    searchMessageByTypes(conversation, keyword, contentTypes, desc, limit, offset) {
        ...
        let result = proto.searchMessageByTypes(JSON.stringify(conversation), keyword, contentTypes, desc, limit, offset);
        let msgs = JSON.parse(result);
        let matchMsgs = [];
        if (msgs && msgs.length > 0) {
            msgs.forEach(m => {
    ...
    searchMessageByTypesAndTimes(conversation, keyword, contentTypes, startTime, endTime, desc, limit, offset) {
        ...
        let result = proto.searchMessageByTypesAndTimes(JSON.stringify(conversation), keyword, contentTypes, startTime.toString(), endTime.toString(), desc, limit, offset);
        let msgs = JSON.parse(result);
        let matchMsgs = [];
        if (msgs && msgs.length > 0) {
            msgs.forEach(m => {
    ...
    searchMessageEx2(conversationTypes, lines, contentTypes, keyword, fromIndex, desc, count) {
      ...
      let result = proto.searchMessageEx2(conversationTypes, lines, contentTypes, keyword, fromIndex, desc, count);
      let msgs = JSON.parse(result);
      let matchMsgs = [];
      if (msgs && msgs.length > 0) {
          msgs.forEach(m => {
    ...
    async updateRemoteMessageContent(messageUid, messageContent, distribute, updateLocal, successCB, failCB) {
        ...
        proto.updateRemoteMessageContent(messageUid.toString(), JSON.stringify(protoMessageContent), distribute, updateLocal,
            () => {
                if (successCB) {
                    successCB();
                }
    ...
    async pinUnpinMessage(message, successCB, failCB) {
        if(typeof message.messageContent.extra !== 'string'){
            message.messageContent.extra.isUpdated = true;
        }else{
            let obj = JSON.parse(message.messageContent.extra)
            obj.isUpdated = true;
            message.messageContent.extra = JSON.stringify(obj);
        }
        let protoMessageContent = message.messageContent.encode();
        proto.updateRemoteMessageContent(stringValue(message.messageUid).toString(), JSON.stringify(protoMessageContent), true, true,
        () => {
            setTimeout(() => {
                if (successCB) {
                    successCB();
    ...
    watchOnlineState(conversationType, targets, duration, successCB, failCB) {
        proto.watchOnlineState(conversationType, targets, duration,
          (strStates) => {
    ...
    unwatchOnlineState(conversationType, targets, successCB, failCB) {
        proto.unwatchOnlineState(conversationType, targets,
          () => {
              if (successCB) {
                  successCB();
              }
    ...
    isGlobalDisableSyncDraft() {
        return proto.isGlobalDisableSyncDraft();
    }

    isEnableUserOnlineState() {
        return proto.isEnableUserOnlineState();
    }
    ...
    isSupportBigFilesUpload() {
        return proto.isSupportBigFilesUpload();
    }

    getUploadMediaUrl(fileName, mediaType, contentType, successCB, failCB) {
        console.log("In proto file", fileName, mediaType, contentType)
        proto.getUploadMediaUrl(fileName, mediaType, contentType, successCB, failCB);
    }

    getConversationFileRecords(conversation, fromUser, beforeUid, order, count, successCB, failCB) {
        proto.getConversationFiles(JSON.stringify(conversation), fromUser, Long.fromValue(beforeUid).toString(), order, count, (frsStr) => {
    ...
    getMyFileRecords(beforeUid, order, count, successCB, failCB) {
        proto.getMyFiles(Long.fromValue(beforeUid).toString(), order, count, (frsStr) => {
            let frs = JSON.parse(frsStr);
            let fileRecords = [];
            frs.forEach(fr => {
                fileRecords.push(this._objStrToFileRecordObj(fr));
    ...
    async clearRemoteConversationMessages(conversation, successCB, failCB) {
        proto.clearRemoteConversationMessages(JSON.stringify(conversation),
            () => {
                successCB && successCB();
            },
            (errorCode) => {
    ...
    insertMessage(conversation, messageContent, status, notify = false, serverTime = 0) {
        let protoMessageContent = messageContent.encode();
        let mStr = proto.insertMessage(JSON.stringify(conversation), self.userId, JSON.stringify(protoMessageContent), status, notify, serverTime);
        return Message.fromProtoMessage(JSON.parse(mStr));
    }
    ...
    searchFiles(keyword, conversation, fromUser, beforeMessageId, order, count, successCB, failCB) {
        proto.searchFiles(keyword, conversation ? JSON.stringify(conversation) : '', fromUser, Long.fromValue(beforeMessageId).toString(), order, count,
            (files) => {
    ...
    searchMyFiles(keyword, beforeMessageUid, order, count, successCB, failCB) {
        proto.searchMyFiles(keyword, Long.fromValue(beforeMessageUid).toString(), order, count,
            (files) => {
                let frs = JSON.parse(files);
